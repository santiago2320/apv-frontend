...
function Formulario() {
    //Registramos los state de los campos del formulario
    ...
    // Acceder a los datos de pacientes (pacientesProvider.)
    const {guardarPaciente, paciente} = usePaciente()

    // Editar Paciente Useeffect carga  cada vez que demos click
    useEffect(()=> {
        if(paciente?.nombre){
            setNombre(paciente.nombre)
            setPropietario(paciente.propietario)
            setEmail(paciente.email)
            setFecha(new Date(paciente.fecha).toLocaleDateString('en-CA')) // Formatear la fecha
    ...
    const handleSubmit = e => {
       ...
       // Si pasa la validación eliminamos la alerta.
  ...
  return (
    <>
        <h2 className="font-black text-3xl text-center"> Administrado de Pacientes</h2>
        ...
        >
            ...
            <div className="mb-5">
                ...
                <input 
                    type="submit"
                    className="bg-indigo-600 w-full p-3 text-white uppercase font-bold rounded-sm hover:bg-indigo-800 cursor-pointer transition-colors" 
